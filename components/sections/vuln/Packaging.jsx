...
export default function Packaging() {
  ...
  const tiers = [0, 1, 2].map((idx) => ({
    name: t(`vuln.packaging.tiers.${idx}.name`),
    points: [
      t(`vuln.packaging.tiers.${idx}.points.0`),
      t(`vuln.packaging.tiers.${idx}.points.1`),
      t(`vuln.packaging.tiers.${idx}.points.2`),
    ],
    cta: t(`vuln.packaging.tiers.${idx}.cta`),
  }));

  return (
    <section className="bg-gray-50 px-6 md:px-10 py-16 md:py-24">
      <div className="max-w-6xl mx-auto text-center">
        <h3 className="text-3xl md:text-4xl font-extrabold text-gray-900">{t("vuln.packaging.title")}</h3>
        <p className="text-gray-600 mt-2 max-w-3xl mx-auto">{t("vuln.packaging.description")}</p>
