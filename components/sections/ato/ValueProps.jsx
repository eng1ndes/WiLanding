...
import { Shield, SearchCheck, ShieldAlert, Mail, ArrowRight } from "lucide-react";

const icons = [Shield, SearchCheck, ShieldAlert, Mail];
...
const containerVariants = {
  ...
  visible: {
    opacity: 1,
    transition: {
      staggerChildren: 0.1, // Stagger effect for premium feel
    },
  },
};
...
export default function ValueProps() {
  ...
  const items = icons.map((icon, i) => ({
    icon,
    title: t(`ato.valueProps.items.${i}.title`),
    desc: t(`ato.valueProps.items.${i}.description`),
  }));
  return (
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-white tracking-tight mb-4">
            {t("ato.valueProps.title")} <span className="text-blue-500">{t("ato.valueProps.titleHighlight")}</span>
          </h2>
          <p className="text-lg text-slate-400">
            {t("ato.valueProps.description")}
          </p>
        </div>
