...
import { Megaphone, FileCode, UserCog, Radar, ArrowRight } from "lucide-react";

const icons = [Megaphone, FileCode, UserCog, Radar];
...
export default function DarkWebValueProps() {
  const { t } = useTranslation("features");

  const items = icons.map((icon, idx) => ({
    icon,
    title: t(`darkweb.valueProps.items.${idx}.title`),
    desc: t(`darkweb.valueProps.items.${idx}.description`),
  }));

  return (
    <section className="relative bg-[#020617] px-6 md:px-10 py-24 border-t border-white/5 overflow-hidden">

      {/* Visual Divider: Purple gradient line to match page theme */}
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-white tracking-tight mb-4">
            {t("darkweb.valueProps.title")} <span className="text-purple-500">{t("darkweb.valueProps.titleHighlight")}</span>
          </h2>
          <p className="text-lg text-slate-400 leading-relaxed">
            {t("darkweb.valueProps.description")}
          </p>
        </div>
