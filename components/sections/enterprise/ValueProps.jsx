...
import { Shield, KeyRound, Users, Lock, ArrowRight } from "lucide-react";

const icons = [Shield, KeyRound, Users, Lock];
...
export default function ValueProps() {
  ...
  const items = icons.map((icon, i) => ({
    icon,
    title: t(`enterprise.valueProps.items.${i}.title`),
    desc: t(`enterprise.valueProps.items.${i}.description`),
  }));
  return (
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-white tracking-tight mb-4">
            {t("enterprise.valueProps.title")} <span className="text-blue-500">{t("enterprise.valueProps.titleHighlight")}</span>
          </h2>
          <p className="text-lg text-slate-400 leading-relaxed">
            {t("enterprise.valueProps.description")}
          </p>
        </div>
